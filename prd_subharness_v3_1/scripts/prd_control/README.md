...
python scripts/prd_control/prototypectl.py --instance-root ../prd_instances/my-product-prd --run-id RUN-001 build-figma-write-handoff --figma-url "https://www.figma.com/design/<file-key>/<file-name>"
...
v3.2 最终写入 Figma 前必须先生成 `semantic_prototype_payload.yaml`，再运行 `build-figma-semantic-writer-input`、`build-figma-semantic-writer-runtime` 和 `build-figma-write-handoff`。真实写入默认 profile 是 `PLAYABLE_PROTOTYPE_WRITE`，writer 只能消费 `figma-semantic-writer-input.full.json`；禁止继续消费 compact payload、DRD board packets、review MD、canvas view model 或只含 `component_id / role / copy` 的压扁数组。`prototype_ui_blueprint.yaml` 和 `playable_prototype_model.yaml` 仍是结构约束源，但不是 writer 的唯一施工输入。最终 `validate-figma-write-readiness` 必须同时看到 semantic payload、semantic writer input、四段模型输出、playable model、coverage、live gate plan 后才会 PASS。`build-figma-write-handoff` 会把可执行 writer runtime、readback runtime、目标 Figma page 名称和禁止回读 PRD/product-spec 的约束固化成 `figma-write-handoff.yaml`；外部 Figma 写入只允许消费该 handoff 指向的 runtime。写入后还必须运行 live readback、geometry、semantic、model visual comprehension 四类审计，并用 `gate-figma-live-visual-audit` 做最终 loop gate。

生成链路拆成 deterministic generator 和 model generator。`generation_job_queue.yaml` 是机器可读队列，每个 job 都必须有 `completion_criteria`；deterministic jobs 生成 source atoms、DRD artifacts、runtime、payload、coverage 和 manifest，model jobs 分别做页面归属、真实链路、状态机和组件蓝图推理。模型调用由 `model_execution_contract.yaml` 约束为只读审查，并由 `model_invocation_trace.yaml` 记录 prompt hash、只读命令、耗时、exit code、raw output path 和 completion criteria。单个模型 stage 默认 timeout 是 900 秒，可用 `PROTOTYPE_MODEL_STAGE_TIMEOUT_SECONDS` 覆盖。
