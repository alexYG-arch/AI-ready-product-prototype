# PRD Sub-Harness v3.1

中文名：AI-ready PRD 子 Harness 架构 3.1
版本：v3.1
定位：在 v2.1 的 **总控 Orchestrator + 多个子 Harness + prototype_projection_harness** 基线之上，新增 `DRD_MODE` 设计评审文档模式的演绎规则归档、可执行 schema、样例 artifact 和只读 CLI 校验。
...
```text
v0.3 像一个全能施工队；
v3.1 像一个总包工头 + 八个专业工种，其中原型 harness 仍只做 projection、renderer staging、diff candidate 和 DRD 投射校验。
```
