...
`prd_subharness_v1_0/` 是 harness 工程包，应该只存放规则、contract、validator、模板和脚本。真实 PRD 的输入、候选、报告、impact、rerun plan、日志和输出必须放在独立 instance 中，避免污染 harness 工程。
...
- Harness 工程包只读：`prd_subharness_v1_0/` 不接收真实 PRD 的运行产物。
...
```bash
python prd_subharness_v1_0/scripts/prd_control/harnessctl.py \
