...
def unified_review(args):
    ...
    lines.extend([
        ...
        "",
        "```bash",
        f"python3 prd_subharness_v1_0/scripts/prd_control/harnessctl.py --instance-root {INSTANCE_ROOT} --run-id {RUN_ID} extract-source-baseline inputs/legacy_prd.md",
        f"python3 prd_subharness_v1_0/scripts/prd_control/harnessctl.py --instance-root {INSTANCE_ROOT} --run-id {RUN_ID} quality-gate",
        f"python3 prd_subharness_v1_0/scripts/prd_control/harnessctl.py --instance-root {INSTANCE_ROOT} --run-id {RUN_ID} validate-prd-template",
        f"python3 prd_subharness_v1_0/scripts/prd_control/harnessctl.py --instance-root {INSTANCE_ROOT} --run-id {RUN_ID} check-prd-sync",
        "```",
    ])
...
def validate_rule_specs(args):
    ...
    for harness_dir in sorted(harness_path("sub_harnesses").glob("*_harness")):
        ...
        if data.get("validator_spec_version") != "1.0":
            issues.append(f"{validators_path}: missing validator_spec_version=1.0")
