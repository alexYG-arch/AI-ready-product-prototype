...
```text
source PRD → extract-source-baseline → extraction review → structured_source_baseline → sub_harness_inputs → CORE-000 → CORE-001 → CORE-002 → REQ-003A → REQ-003B → REQ-003C → SCREEN-004 → DATA-005 → TRACE-006 → PROJ-007
```
...
```text
change_request.md → CHANGE-INGEST → route to sub-harness → PRD-DELTA → impact → rerun-plan → PROJ sync
```
