# 架构说明 v2.1：Prototype Projection Harness

## 1. 设计目标

v2.1 在 v2.0 的 PRD sub-harness 架构上深化 Figma 原型投射能力。目标不是做一个独立于 PRD 的设计稿生成器，而是建立：

```text
structured PRD facts
→ prototype runtime model
→ renderer stage plan
→ Figma frames / components / reactions / comments / flow map
```
...
v2.1 保留 v2.0 新增的投影域，并细化 prototype projection 内部 stage：
...
prototype.runtime.base.json
        ↓
prototype.runtime.json with generation_lineage / reasoning_completion
        ↓
UX writer copy catalog + layout route plan
        ↓
Figma projection
```
...
3. Documentation projection
   生成 Flow Map / Interaction Spec / Figma comments / visual pen lines，解释 trigger、guard、action chain、event、source_refs。
```
...
Figma 中说明箭头 / comments / visual lines = documentation，不等于真实交互。
```
...
v2.1 的原型能力必须按 stage 保留和校验，不能把规则压平成单一说明文档：

```text
Stage 1: 注册 prototype_projection_harness 和文档权威边界
Stage 2: 从 structured facts 生成 prototype.runtime.base.json，不写 Figma
Stage 3: 完成 reasoning_completion，生成 completed prototype.runtime.json
Stage 4: 执行 RND-00 到 RND-09 renderer stage plan
Stage 5: 接入 UX writer copy、真实 component / pattern binding，无法绑定时生成 gap
Stage 6: 写 Figma reactions、comments 和 documentation-only visual lines
Stage 7: Figma semantic diff → figma_diff_report → change_patch_candidate → owner harness review
```
...
PROTO-012 runtime reasoning completion
PROTO-013 renderer staged implementation
PROTO-014 Figma comment annotations
PROTO-015 UX writer copy stage
PROTO-016 layout routing and visual line coupling
PROTO-017 renderer skill hook
```
...
```text
RND-00-input-freeze
RND-01-layout-route-blueprint
RND-02-canvas-skeleton
RND-03-sequence-group-page-render
RND-04-component-and-pattern-fill
RND-05-copy-application
RND-06-interaction-comments
RND-07-prototype-reactions
RND-08-visual-flow-lines
RND-09-final-validation
```
