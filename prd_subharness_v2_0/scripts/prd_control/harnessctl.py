...
def validate_rule_specs(args):
    ...
    for harness_dir in sorted(harness_path("sub_harnesses").glob("*_harness")):
        ...
        expected_spec = "2.0" if grouped_validators else "1.0"
        if data.get("validator_spec_version") != expected_spec:
            issues.append(f"{validators_path}: missing validator_spec_version={expected_spec}")
        ...
        def inspect_rule_node(path, node):
            ...
            if isinstance(node, dict):
                ...
                if isinstance(patch_contract, dict):
                    ...
                    if patch_contract.get("schema_version") == "2.0":
